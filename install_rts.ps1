...
Out-Logging -out $FileLog -src $MyInvocation.MyCommand.Name -m "Install RTS base pack... "

if( Test-Path -Path $SOFT_INST\rts\base.zip ) {
	...
	CMD_ErrCr
	Out-Logging -out $FileLog -src $MyInvocation.MyCommand.Name -t e -m "File $SOFT_INST\rts\base.zip not found" -cr
}

$Version = $jsonCFG.RTS.version
if( !$Force ){
	Write-Host
...
Out-Logging -out $FileLog -src $MyInvocation.MyCommand.Name -m "Install RTS Release pack... "
if( Test-Path -Path $SOFT_INST\rts\Release.$Version.zip ) {
	Expand-Archive -LiteralPath $SOFT_INST\rts\Release.$Version.zip -Destination "$($Data.PATH_PELENG)" -Force
	if( $? ){ CMD_OkCr } else {
		CMD_ErrCr
		Out-Logging -out $FileLog -src $MyInvocation.MyCommand.Name -t e -m "Unpacking error" -cr
	}
